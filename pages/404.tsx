  ...
  return (
    <>
      ...
      >
        ...
        <span style={{ marginTop: '48px', marginBottom: '24px' }}>Were you looking for the following pages?</span>
        <div style={{ marginBottom: '48px', display: 'flex', gap: '24px' }}>
          ...
          <Link href="/blog" passHref>
            <Button size="large" variant="outlined" startIcon={<MenuBookIcon />}>
              Blog
            </Button>
          </Link>
        </div>
        <span style={{ marginBottom: '24px' }}>Perhaps you want to learn about:</span>
        <Stack direction="row" spacing={1}>
          {allTopicLogos.map((topic: TopicLogo, index: number) => (
            <Link key={index} href={`/learn/${topic.topic.toLowerCase()}/introduction`} passHref>
              <Chip className="chip-tag" label={topic.topic} variant="outlined" />
            </Link>
          ))}
        </Stack>
      </div>
