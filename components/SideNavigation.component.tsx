...
const SideNavigationComponent: FC<Props> = ({ menu, slug, metaData }) => {
  ...
  const getInitialMenu = (menu: Menu) => menu.map((menuItem: MenuItem) => ({ ...menuItem, isOpen: false }))
  ...
  const getMenuItemStyle = (v1: string, v2: string = slug) => ({
    color: v1?.toLowerCase() === v2.toLowerCase() ? customTheme.palette.primary.main : customTheme.palette.text.primary,
    cursor: 'pointer',
  })

  const getIntroductionLI = () => {
    return (
      <Link href={`/learn/${metaData.section.toLowerCase()}/introduction`} passHref>
        <ListItem className="list-item" key={-1}>
          <ListItemText primary="Introduction" style={getMenuItemStyle('introduction')} />
        </ListItem>
      </Link>
    )
  }
          ...
          <Box sx={{ overflow: 'auto', marginTop: '48px' }}>
            ...
            <List>
              {activeMenu.map((menuItem: any, index: number) => {
                let [[label, subMenu], [isOpenKey, isOpen]] = Object.entries(menuItem)
                ...
                const section2 = (
                  <Link key={index} href={href} passHref>
                    <ListItem
                      ...
                      secondaryAction={
                        (Object.values(menuItem)[0] as string[]).length > 0 && (
                          <>
                            {menuItem.isOpen ? (
                              <KeyboardArrowUpIcon style={{ cursor: 'pointer' }} color="primary" />
                            ) : (
                              <KeyboardArrowDownIcon style={{ cursor: 'pointer' }} color="primary" />
                            )}
                          </>
                        )
                      }
                    >
                      <ListItemText
                        primary={label}
                        style={getMenuItemStyle(metaData?.subSection || metaData.topic, label)}
                      />
                    </ListItem>
                  </Link>
                )

                const subSection2 = (subMenu as string[]).map((item: string, index: number) => {
                  return (
                    menuItem.isOpen && (
                      <Link
                        key={label + '_' + index}
                      ...
                      >
                        <ListItem className="list-item">
                          <ListItemText primary={'> ' + item} style={getMenuItemStyle(item)} />
                        </ListItem>
                      </Link>
                    )
                  )
